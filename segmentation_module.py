...
from modules import DeeplabV3, SegFormer_Body, SegFormer_Head, segformer_type_args
...
def make_model(opts, classes=None):
    ...
    elif opts.model.startswith('segformer'):
        segformer_args = segformer_type_args[opts.model]
        segformer_args['num_classes'] = opts.num_classes

        body = SegFormer_Body(**segformer_args)
        head = SegFormer_Head(**segformer_args)
        head_channels = segformer_args['decoder_dim']
        print('segformer args:', segformer_args)
    else:
...
class IncrementalSegmentationModule(nn.Module):
    ...
    def _network(self, x, ret_intermediate=False, only_bg=False):
        if self.opts.model == 'deeplab':
            x_b, attentions, branch1_x, branch2_x = self.body(x)
            x_pl = self.head(x_b)
        else:
            features, ret_attn = self.body(x)
            x_pl, feature_map = self.head(features)
            x_b = features[-1]
            attentions = features[:-1]
